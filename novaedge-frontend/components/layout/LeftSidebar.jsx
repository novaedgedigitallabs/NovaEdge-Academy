...
// ─── Memoized NavLink: only re-renders when its own active state or badge changes ───
const NavLink = memo(function NavLink({ icon: IconComponent, route, label, badgeCount }) {
...
// ─── Memoized LogoutButton: only re-renders when logout function changes ───
...
// ─── PostDialog: isolates dialog open/close state from parent ───
const PostDialog = memo(function PostDialog() {
    const [isPostDialogOpen, setIsPostDialogOpen] = useState(false);

    const handleOpen = useCallback(() => setIsPostDialogOpen(true), []);
    const handleClose = useCallback(() => setIsPostDialogOpen(false), []);
    ...
    return (
        <Dialog open={isPostDialogOpen} onOpenChange={setIsPostDialogOpen}>
            ...
            <DialogContent className="sm:max-w-xl p-0 border border-border bg-background rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div className="p-2">
                    <CreatePost onPostCreated={handleClose} />
                </div>
            </DialogContent>
        </Dialog>
    );
...
// ─── UserProfile: only re-renders when user object changes ───
...
// ─── Main LeftSidebar: mostly static shell ───
export default function LeftSidebar() {
    const { user, logout } = useAuth();
    const [pendingRequestsCount, setPendingRequestsCount] = useState(0);

    // Fetch friend requests only when user changes (not on every route change)
    useEffect(() => {
        if (user) {
            getFriendRequests()
                .then((res) => {
                ...
                <div className="flex flex-col gap-1.5">
                    {sidebarLinks.map((link) => (
                        <NavLink
                            ...
                            route={link.route}
                            label={link.label}
                            badgeCount={link.badgeKey === "requests" ? pendingRequestsCount : 0}
                        />
                    ))}
