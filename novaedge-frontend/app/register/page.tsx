...
export function RegisterForm() {
  ...
  const referralCode = searchParams?.get ? searchParams.get("ref") : null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await register(email, name, password, referralCode, username, phoneNumber);
      if (res && !res.ok) {
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-sm space-y-6">
          ...
          {error && (
            <div className="p-3 text-sm font-medium rounded-md bg-destructive/15 text-destructive border border-destructive/20">
              {error}
            </div>
          )}
